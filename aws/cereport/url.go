...
// its URLs to the dimension keys the Cost Explorer API expects. Unmapped ids
...
func ParseURL(raw string) (*Spec, error) {
	...
	spec := &Spec{
		...
		ReportARN:  q.Get("reportArn"),
		ChartStyle: q.Get("chartStyle"),
		TimeRange: TimeRange{
	...
	if q.Get("useNormalizedUnits") == "true" {
		spec.Metric = "NormalizedUsageAmount"
	} else {
		m, ok := consoleMetrics[agg]
		if !ok {
			return nil, fmt.Errorf("unknown costAggregate %q (known: %s)", agg, sortedKeys(consoleMetrics))
		}
		spec.Metric = m
	}
	...
	if !ok {
		return nil, fmt.Errorf("unknown granularity %q (known: %s)", q.Get("granularity"), sortedKeys(consoleGranularities))
	}
	spec.Granularity = g
...
func parseFilters(raw string) ([]Filter, error) {
	...
	for _, cf := range cfs {
		...
		if cf.Dimension.ID == consoleTagFilterID {
			...
			f = Filter{Type: "TAG", Key: cf.GrowableValue.Value}
		} else {
...
func resolveKey(id string) (Group, error) {
	if k, ok := strings.CutPrefix(id, consoleTagGroupPrefix); ok {
		...
		return Group{Type: "TAG", Key: k}, nil
	}
	if dim, ok := consoleDimensions[id]; ok {
		return Group{Type: "DIMENSION", Key: dim}, nil
	}
